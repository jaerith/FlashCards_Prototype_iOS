...
#import "PrototypeTwoAnswerController.h"
#import "PrototypeTwoResultsController.h"

@implementation PrototypeTwoCardsController
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
	...
	if (self = [super initWithNibName:@"PrototypeTwoCards" bundle:mainBundle]) {
...
- (void)viewDidLoad {
	...
	PrototypeTwoQuestionController *tmpQuestionController =
		[[PrototypeTwoQuestionController alloc] initWithNibName:@"PrototypeTwoQuestion" bundle:nil];
	
	[tmpQuestionController setParent:self];
	
	self.questionController = tmpQuestionController;
	
	PrototypeTwoAnswerController *tmpAnswerController =
		[[PrototypeTwoAnswerController alloc] initWithNibName:@"PrototypeTwoAnswer" bundle:nil];
	...
	[self.parentController moveToCardView];
...
- (void) showResults {
	...
	PrototypeTwoResultsController *resultsController = 
		[[PrototypeTwoResultsController alloc] init];
	
	[resultsController setParent:self.parentController];
...
-(IBAction)nextCard:(id)sender {
	...
	if (nCurrPage < (nCount-1)) {
		...
		[self curlPageForward:self];
	}
}

-(IBAction)curlPageBack:(id)sender {

	[UIView beginAnimations:@"View Curl Back" context:nil];
...
-(IBAction)curlPageForward:(id)sender {
	...
	[UIView beginAnimations:@"View Curl Forward" context:nil];
	[UIView setAnimationDuration:0.50];
	[UIView setAnimationCurve:UIViewAnimationCurveEaseInOut];	
